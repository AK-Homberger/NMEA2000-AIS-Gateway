...
Only AIVDM messages (other ship) are decoded by default. If you also want to decode and forward own ship messages (AIVDO) comment/uncomment the appropriate lines:
...
## Updates:
Select if AIVDO messages (own ship) are to be decoded also.
- 13.01.22 Version 0.7: Select if AIVDO messages (own ship) are to be decoded also in addtion to AIVDM.
